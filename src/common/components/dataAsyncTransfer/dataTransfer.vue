...
export default {
    ...
    return {
      ...
      renderFunc(h, option) {
        return h(
          ...
          option.account === "$no_data_display"
            ? option.name
  ...
  methods: {
    filterData(condition, item) {
      if (
        condition !== this.leftCondition &&
        !this.rightDataKeys.includes(item.account)
      ) {
        this.asyncQueryUser(condition);
        this.leftCondition = condition;
      } else if (condition !== this.rightCondition) {
        return (
          item.account.indexOf(condition) > -1 ||
    ...
    queryUserByStr(str) {
      const scope = this;
      scope.$http
        .post(scope.appContextPath + scope.leftQueryUrl, {
          page: 1,
          rows: 50,
          bo: {
            account: str || "",
        ...
        .then(success => {
          if (success.data && success.data.bo && success.data.bo.rows) {
            scope.leftData = success.data.bo.rows
              .filter(e => !scope.rightDataKeys.includes(e.account))
              .concat(scope.rightData);
    ...
    initData() {
      ...
      const scope = this;
      this.$http
        .post(this.appContextPath + this.rightQueryUrl, {
          bo: this.params,
          order: "desc",
          sort: "name"
        })
    ...
    saveSelect() {
      ...
      const scope = this;
      this.$http
        .post(this.appContextPath + this.saveUrl, {
          bo: this.params,
          other: {
    ...
    changeRightData(value, side, checked) {
      if (side === "right") {
        this.rightData = this.rightData.concat(
          this.leftData.filter(e => checked.includes(e.account))
        );
      } else if (side === "left") {
        this.leftData = this.leftData.concat(
          this.rightData.filter(e => checked.includes(e.account))
        );
      }
    }
  },
