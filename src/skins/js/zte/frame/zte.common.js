...
//		  var root = "http://10.5.172.145:8080/";
		//var root = "http://10.89.208.188/";
		//部署的webapi服务上下文,需要根据业务系统实际情况修改
		var rootContext ="Form_002_Define"
	...
	var tabStorage;
	var tabFrame = getTabFrame();
	if(tabFrame){
		tabStorage = window.top.$(tabFrame).data('tabStorage');		
		if(!tabStorage){
			tabStorage = {}
			window.top.$(tabFrame).data('tabStorage',tabStorage);
		}
	}
	function getTabFrame() {
		if ( !window.frameElement) return null;//顶级窗口或404时frameElement是null
		var tabFrame = window.frameElement;
		while(tabFrame && $(tabFrame).parent().attr('role') != 'tabpanel'){
			try{
				tabFrame = tabFrame.contentWindow.parent.frameElement;
			}catch(e){
				console.warn('获取tab页窗口对象时报错');
				tabFrame = null;
			}
		}
		return tabFrame;
	}
	//tab页内的storage，不同tab页之间互不干涉。
	zte.tabStorage = {
		
		put:function(key,data){
			if (!tabStorage) {
				console.error('zte.tabStorage.put方法只能在标签页面中使用');
				return;
			}
			console.log('tabStorage put '+key+' data');
			tabStorage[key] = data;
		},
		...
		get:function(key){
			...
			if (!tabStorage) {
				console.error('zte.tabStorage.get方法只能在标签页面中使用');
				return;
			}
			console.log('tabStorage get '+key+' data');
			
			var data = tabStorage[key];
	...
	zte.datatable.lang = {    
	  "aLengthMenu": [[5, 10,20, 50, 100,-1],[5,10,20,50,100, "全部"]],
	  "oLanguage": {   
		  "sProcessing":   "处理中...",
